...
scale_fill_rp <- function(...) {
  scale_fill_gradientn(
    colors = c("#1a3a5c", "#1d6fa4", "#4ba3c7", "#a8d5e2", "#f5e6a3", "#e8b84b", "#c47c0f"),
    ...
  )
}
```
...
LiDAR point clouds frequently contain data voids where the sensor received no usable return, typically under dense vegetation, at steep angles, or near flight strip edges. Left unfilled, these holes disrupt the gradient computation in Step 2.
...
**`w`** (default `15`): The focal window size in pixels. Larger windows can fill bigger voids but may pull values from more distant and less representative neighbors.
...
4. **Morphological closing** — a dilate-then-erode operation bridges small gaps in the detected edge lines, producing closed loops that surround building interiors
...
ggplot() +
  ...
  geom_sf(data = buildings_sf, fill = NA, color = "#c47c0f", linewidth = 0.6) +
  coord_sf() +
  scale_fill_rp(name = "Elev. (m)") +
  theme_rp() +
  labs(
...
ggplot() +
  geom_raster(data = r_df, aes(x = x, y = y, fill = value)) +
  geom_sf(data = filtered, fill = NA, color = "#c47c0f", linewidth = 0.6) +
  coord_sf() +
  scale_fill_rp(name = "Elev. (m)") +
  theme_rp() +
  labs(
...
ggplot() +
  geom_raster(data = r_df, aes(x = x, y = y, fill = value)) +
  geom_sf(data = valid_polys, fill = NA, color = "#c47c0f", linewidth = 0.6) +
  coord_sf() +
  scale_fill_rp(name = "Elev. (m)") +
  theme_rp() +
  labs(
...
Using a low percentile (default `prob = 0.02`, the 2nd percentile) rather than the mean makes the estimate robust to rooftop features like parapets, HVAC equipment, and other protrusions that would inflate a mean-based estimate. The same percentile is used for both inside and outside values for consistency. If `clamp = TRUE`, negative heights (which can arise from noisy data or polygon edge effects) are set to zero.
...
Each RANSAC iteration randomly draws 3 pixels from the building's raster footprint, fits a plane through them via linear regression (`z ~ x + y`), and counts how many of the *remaining* pixels (inliers) have residuals below `thresh`. After `n_iter` iterations, the plane that maximized inlier count is selected and used to compute final slope and aspect:
...
**`min_inliers`** (default `10`): Minimum pixels required to accept any candidate plane. For small buildings with few raster cells, lower this to avoid discarding valid fits. For large buildings, raising it ensures the plane is well-supported across the full footprint.

**`n_iter`** (default `500`): Number of random 3-point samples evaluated. More iterations improve the chance of finding the globally best plane. `200` is often sufficient for clean data; `1000` or more is warranted for noisy or geometrically complex roofs.
